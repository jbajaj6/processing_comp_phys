...
def create_balls():
    ...
    for i in population:
        mass = 4.2
        rad = .125
        h = Vec(30.48*cos(deg_to_rad(i.angle)),30.48*sin(deg_to_rad(i.angle)),0.0) #ask if length of cannon changes initial position based on angle
        #h = Vec(0.0,0.0,0.0)
        v = 330.0
        a = Vec(0.0,0.0,0.0)
        spin = None
        spin_attr = None
        b = Sphere(rad,mass,0.01, h,Vec(v*cos(deg_to_rad(i.angle)),v*sin(deg_to_rad(i.angle)),0.0),a, spin, spin_attr,i.id)
        balls.append(b)
